...
**Total files to move:** 47 source files + 3 CSS files
...
#### 1d. `packages/remyx-core/src/index.js`
...
- Run `cd packages/remyx-core && npm run build`
- Confirm `dist/remyx-core.js`, `dist/remyx-core.cjs`, and `dist/style.css` are generated
- Confirm no React imports in any file (grep for `from 'react'`)
...
```
Step 1  → @remyx/core             (MUST be first — all others depend on it)
...
| --- | --- | --- | --- | --- |
| 1 | `@remyx/core` | Large | 2-3 | P0 — Required |
